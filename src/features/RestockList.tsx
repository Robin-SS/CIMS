...
type SortKey = 'name' | 'inStock' | 'threshold' | 'usedThisMonth' | 'status';
  ...
  const sortedItems = useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        // Handle numeric values (inStock, threshold, usedThisMonth)
        if (typeof aValue === 'number' && typeof bValue === 'number') {
          return sortConfig.direction === 'asc' ? aValue - bValue : bValue - aValue;
        }

        // Handle string values (name, status)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1.5fr 1.5fr 1.5fr', padding: '12px 16px', background: '#FFFFFF', borderBottom: '2px solid #f2d8c3', fontSize: 10, fontWeight: 700, color: '#D1915F', textTransform: 'uppercase', letterSpacing: 0.5 }}>
        ...
        <div onClick={() => handleSort('inStock')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none' }}>
          IN STOCK {renderSortIcon('inStock')}
        </div>
        
        <div onClick={() => handleSort('threshold')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none' }}>
          REORDER AT {renderSortIcon('threshold')}
        </div>
        
        <div onClick={() => handleSort('usedThisMonth')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none' }}>
          USED THIS MONTH {renderSortIcon('usedThisMonth')}
        </div>
        
        <div onClick={() => handleSort('status')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', userSelect: 'none' }}>
